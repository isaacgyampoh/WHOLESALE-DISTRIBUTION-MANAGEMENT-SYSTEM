...
const UPGRADE_VIEW = path.join("..", "..", "database", "UPGRADE_0038_PRICED_VIEW.sql");
const UPGRADE_USERS = path.join("..", "..", "database", "UPGRADE_0039_USERNAMES.sql");
const UPGRADE_LIMIT = path.join("..", "..", "database", "UPGRADE_0040_ATTEMPT_LIMIT.sql");
const UPGRADE_RECEIPTS = path.join("..", "..", "database", "UPGRADE_0041_RECEIPTS.sql");
const UPGRADE_VANAUTH = path.join("..", "..", "database", "UPGRADE_0042_VAN_AUTHORITY.sql");
...
for (const [name, file] of [["0029", UPGRADE_DOCS_SUP], ["0030", UPGRADE_PORTAL],
                            ...
                            ["0038", UPGRADE_VIEW], ["0039", UPGRADE_USERS],
                            ["0040", UPGRADE_LIMIT], ["0041", UPGRADE_RECEIPTS], ["0042", UPGRADE_VANAUTH]]) {
...
for (const [what, sql] of [
  ...
  // A tenant can be removed at all: the ledger used to refuse every
  // delete, which left the cleanup half-finished.
  ["a tenant purge is possible", `select position('is_trusted_context' in
      (select pg_get_functiondef(p.oid) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
        where n.nspname='public' and p.proname='block_movement_mutation' limit 1)) > 0 v`],
  // Every function the salesperson needs knows the role exists. Without
  // this, credit selling and offline sync were both refused outright.
  ["a salesperson can raise an invoice", `select position('salesperson' in
      (select pg_get_functiondef(p.oid) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
        where n.nspname='public' and p.proname='issue_invoice_for_sale' limit 1)) > 0 v`],
