-- ===================================================================
-- 0032  A van has a crew: a driver, and the people who sell
-- ===================================================================
--
-- The schema has always assumed the driver is the salesperson. Every
-- van record is keyed on `driver_id`: the assignment, the load, the
-- sale, the return, the reconciliation. `my_van_id()` resolves a van by
-- asking which one the caller drives, and every row level security
-- policy for field staff is built on that.
--
-- That is not how the business works. A van goes out with a driver who
-- drives it and one or more salespeople who sell from it. Those are
-- different people doing different jobs, and the money is handled by the
-- second.
--
-- Renaming the role would leave the model unchanged and the wrong person
-- named on every receipt. So this changes the model:
--
--   van_assignments becomes a crew list - a member and the job they do
--   on that van - rather than a driver slot. One driver per van, any
--   number of salespeople.
--
--   my_van_id() resolves for any crew member, so a salesperson sees the
--   van's stock and the policies that depend on it keep working.
--
--   a sale records who sold it and who drove, separately. Existing sales
--   are backfilled with the driver as the salesperson, because on those
--   rounds that person genuinely was both.
--
-- Nothing historical is discarded. Every existing assignment becomes a
-- driver assignment, which is what it was.

-- ------------------------------------------------------------------
-- The jobs a crew member can hold
-- ------------------------------------------------------------------
...
drop trigger if exists van_assignments_check_member on public.van_assignments;
...
drop trigger if exists van_sales_fill_crew on public.van_sales;
...
drop policy if exists van_load_crew_read on public.van_load_crew;
...
drop policy if exists van_load_crew_write on public.van_load_crew;
...
drop trigger if exists van_loads_snapshot_crew on public.van_loads;
