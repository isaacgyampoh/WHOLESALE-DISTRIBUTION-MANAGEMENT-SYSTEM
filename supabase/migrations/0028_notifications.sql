-- ===================================================================
-- 0028  Telling people what needs them
-- ===================================================================
--
-- Everything this system knows is on a screen somewhere, and that is the
-- problem: a reconciliation submitted at seven in the evening sits on
-- the reconciliation screen, and nobody opens the reconciliation screen
-- unless they already suspect there is something on it.
--
-- Two different things get called a notification, and they behave
-- differently, so both are here rather than one pretending to be the
-- other:
--
--   an EVENT happened once and stays true. A driver submitted an end of
--   day. A transfer is waiting for approval. These are written when the
--   thing happens, by trigger, and a person marks them read.
--
--   a CONDITION is true until it is not. Stock is below its reorder
--   point. An invoice is overdue. Nobody 'reads' one of these - it goes
--   away when the stock is replenished or the invoice is paid. Writing
--   one row per day would bury the person under repeats of the same
--   fact.
--
-- Conditions are therefore refreshed rather than appended: one row per
-- (organization, kind, subject), updated in place, and cleared when the
-- condition ends. Refreshing is a function the dashboard calls, so this
-- needs no scheduler and works on a database with no cron.
...
drop trigger if exists reconciliations_notify on public.van_reconciliations;
...
drop trigger if exists van_returns_notify on public.van_returns;
...
drop trigger if exists stock_transfers_notify_short on public.stock_transfers;
