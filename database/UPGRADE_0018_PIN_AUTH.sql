...
-- GAB Premium Ent
-- Upgrade 0018: four-digit PIN sign-in.
-- For a database that is ALREADY INSTALLED at migration 0017.
--
-- WHAT IT ADDS
--   1. profiles.pin_hash, holding an HMAC of the PIN under a secret that
--      lives only in the server environment. A copy of this database is
--      not enough to recover anyone's PIN.
--   2. A unique index so no two active people share a PIN.
--   3. auth_pin_attempts, the sign-in attempt log used for rate
--      limiting. It holds no PIN and no digest, and is withheld from
--      anon and authenticated entirely.
--
--   No business data is touched. No table is dropped. Existing accounts
--   keep their role, organization and active state.
--
-- BEFORE RUNNING IT
--   Set PIN_PEPPER in the server environment, or nobody can sign in:
--     openssl rand -hex 32
--   Changing that value later invalidates every PIN.
--
-- AFTER RUNNING IT
--   Existing accounts have no PIN and cannot sign in until one is set.
--   Seed the demonstration organization, which creates working accounts:
--     npm run demo:seed
--   Or set one by hand, using the digest the application computes. The
--   simplest route is to seed the demo, sign in as the Super
--   Administrator, and use Staff to give everyone else a PIN.
--
-- HOW TO RUN
