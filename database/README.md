...
| `UPGRADE_0032_SALESPERSON_ROLE.sql` | The salesperson role. **Run this on its own, before 0033** — PostgreSQL will not let 0033 use a label added in the same transaction. |
| `UPGRADE_0033_VAN_CREW.sql` | A van has a crew: a driver and the people who sell from it. |
| `UPGRADE_0034_MOMO_PROVIDER.sql` | Which mobile money network a payment came from, and reconciliation by it. |
| `UPGRADE_0035_LEDGER_PURGE.sql` | Lets a trusted role clear a tenant's stock movements, so an organization can actually be removed. |
...
everything up to and including 0035. The upgrade scripts are for a
...
## `GAB_PREMIUM_ENT_DATABASE.sql`

The same file as `WHOLESALE_DISTRIBUTION_DATABASE.sql`, under the name
the business asked for. Both are generated by `node database/build.mjs`
from `supabase/migrations/` — never edited by hand. If they disagree,
regenerate.

## `PRODUCTION_CLEAN.sql`

Removes the demonstration organization and everything in it, scoped by
slug throughout. Prefer `npm run production:clean`, which shows you what
it would delete first and can also remove the Supabase Auth accounts —
SQL cannot, because those live in the `auth` schema.
